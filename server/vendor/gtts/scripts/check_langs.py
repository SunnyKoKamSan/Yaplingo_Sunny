...
try:
    ...
    from langs_temp import _langs as _new_langs  # type: ignore[reportMissingImports]
except ImportError:
...
if modified and not added and not removed:
    ...
    messages = []
    if added:
        messages.append(f"language{'s' if len(added_dict) > 1 else ''} added: {added_markdown}")
    if removed:
        messages.append(f"language{'s' if len(removed_dict) > 1 else ''} removed: {removed_markdown}")
    if modified:
        messages.append(f"language{'s' if len(modified_dict) > 1 else ''} modified: {modified_markdown}")
