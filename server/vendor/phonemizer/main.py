...
class CatchExceptions:  # pragma: nocover
    ...
    def __call__(self):
        ...
        except (IOError, ValueError, OSError, RuntimeError, AssertionError) as err:
...
* Phonemize some French text file using espeak
  ...
  $ phonemize -l fr-fr -b espeak text.txt -o phones.txt
    ...
    group.add_argument(
        ...
        "--verbose",
        action="store_true",
        help="write all log messages to stderr (displays only warnings by default).",
    )
    group.add_argument("-q", "--quiet", action="store_true", help="do not display any log message, even warnings.")

    parser.add_argument(
        "-j", "--njobs", type=int, metavar="<int>", default=1, help="number of parallel jobs, default is %(default)s."
    )
    ...
    group.add_argument("--strip", action="store_true", help="removes the end separators in phonemized tokens.")
    ...
    group = parser.add_argument_group("punctuation processing", description="not available for espeak-mbrola backend")
    group.add_argument(
...
def list_languages(args_backend):
    ...
    for backend in BACKENDS.keys() if not args_backend else [args_backend]:
        print(
            ...
            + "\n".join(f"\t{k}\t->\t{v}" for k, v in sorted(BACKENDS[backend].supported_languages().items()))
        )
...
def main():
    ...
    if args.backend == "espeak-mbrola":
        ...
        sep = separator.Separator(phone=args.phone_separator, syllable=None, word=None)
    else:
        sep = separator.Separator(
            phone=args.phone_separator, syllable=args.syllable_separator, word=args.word_separator
        )
    log.debug("separator is %s", sep)

    if args.prepend_text:
        input_output_separator = sep.input_output_separator(args.prepend_text)
        log.debug('prepend input text to output, separator is "%s"', input_output_separator)
    else:
    ...
    if out and input_output_separator:
        streamout.write(os.linesep.join(f"{line[0]} {input_output_separator} {line[1]}" for line in out) + os.linesep)
    elif out:
