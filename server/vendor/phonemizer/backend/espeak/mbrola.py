...
class EspeakMbrolaBackend(BaseEspeakBackend):
    ...
    def is_available(cls) -> bool:
        ...
        return BaseEspeakBackend.is_available() and shutil.which("mbrola") and BaseEspeakBackend.is_espeak_ng()
    ...
    @classmethod
    def _is_language_installed(cls, language: str, data_path: Union[str, Path]) -> bool:
    ...
    def supported_languages(cls) -> Dict[str, str]:  # pragma: nocover
        ...
        if cls._supported_languages is None:
            ...
            cls._supported_languages = {
                k: v for k, v in cls._all_supported_languages().items() if cls._is_language_installed(k, data_path)
            }
    ...
    def _postprocess_line(self, line: str, num: int, separator: Separator, strip: bool) -> str:
        # retrieve the phonemes with the correct SAMPA alphabet (but
        # without word separation)
        phonemes = (phn.split("\t")[0] for phn in line.split("\n") if phn.strip())
