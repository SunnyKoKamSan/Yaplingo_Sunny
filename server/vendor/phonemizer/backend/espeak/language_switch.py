...
def get_language_switch_processor(mode: LanguageSwitch, logger: Logger, language: str) -> "BaseLanguageSwitch":
    ...
    """
    processors = {"keep-flags": KeepFlags, "remove-flags": RemoveFlags, "remove-utterance": RemoveUtterances}

    try:
        return processors[mode](logger, language)
    except KeyError:
        raise RuntimeError(f'mode "{mode}" invalid, must be in {", ".join(processors.keys())}') from None
...
class KeepFlags(BaseLanguageSwitch):
    ...
    def warning(self, switches: List[int]):
        ...
        self._logger.warning(
            "%s utterances containing language switches on lines %s",
            nswitches,
            ", ".join(str(switch) for switch in sorted(switches)),
        )
        self._logger.warning('extra phones may appear in the "%s" phoneset', self._language)
        self._logger.warning('language switch flags have been kept (applying "keep-flags" policy)')
...
class RemoveFlags(BaseLanguageSwitch):
    ...
    def warning(self, switches: List[int]):
        ...
        self._logger.warning('extra phones may appear in the "%s" phoneset', self._language)
        self._logger.warning('language switch flags have been removed (applying "remove-flags" policy)')
...
class RemoveUtterances(BaseLanguageSwitch):
    ...
    def warning(self, switches: List[int]):
        ...
        nswitches = len(switches)
        self._logger.warning(
            'removed %s utterances containing language switches (applying "remove-utterance" policy)', nswitches
        )
