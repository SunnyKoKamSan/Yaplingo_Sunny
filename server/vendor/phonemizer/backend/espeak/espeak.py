...
class EspeakBackend(BaseEspeakBackend):
    ...
    def __init__(
        ...
        self._words_mismatch: BaseWordsMismatch = get_words_mismatch_processor(words_mismatch, self.logger)
    ...
    def supported_languages(cls):
        return {voice.language: voice.name for voice in EspeakWrapper().available_voices()}

    def _phonemize_aux(self, text, offset, separator, strip):
        if self._tie is not None and separator.phone:
            self.logger.warning("cannot use ties AND phone separation, ignoring phone separator")
    ...
    def _postprocess_line(self, line: str, num: int, separator: Separator, strip: bool) -> Tuple[str, bool]:
    ...
    def _flatten(phonemized) -> List:
        ...
        for i in range(len(phonemized[0])):
            flattened.append(list(itertools.chain(c for chunk in phonemized for c in chunk[i])))
        return flattened
