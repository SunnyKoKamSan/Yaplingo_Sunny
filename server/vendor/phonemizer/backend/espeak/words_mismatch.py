...
def get_words_mismatch_processor(mode: WordMismatch, logger: Logger) -> "BaseWordsMismatch":
    ...
    """
    processors = {"ignore": Ignore, "warn": Warn, "remove": Remove}
...
class BaseWordsMismatch(abc.ABC):
    ...
    @classmethod
    def _count_words(cls, text: List[str], wordsep: Union[str, re.Pattern] = _RE_SPACES) -> List[int]:
        """Return the number of words contained in each line of `text`"""
        if not isinstance(wordsep, re.Pattern):
            wordsep = re.escape(wordsep)

        return [len([w for w in re.split(wordsep, line.strip()) if w]) for line in text]
    ...
    def _mismatched_lines(self) -> List[Tuple[int, int, int]]:
        ...
        return [(n, t, p) for n, (t, p) in enumerate(zip(self._count_txt, self._count_phn)) if t != p]
...
class Warn(BaseWordsMismatch):
    ...
    def process(self, text: List[str]) -> List[str]:
        ...
        for num, ntxt, nphn in mismatch:
            self._logger.warning("words count mismatch on line %s (expected %s words but get %s)", num + 1, ntxt, nphn)
