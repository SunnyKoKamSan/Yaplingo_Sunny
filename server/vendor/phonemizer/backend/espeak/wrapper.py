...
class EspeakWrapper:
    ...
    def __init__(self):
        ...
        # load the espeak API
        self._espeak = EspeakAPI(self.library(), self._data_path)
    ...
    def __getstate__(self):
        ...
        return {"version": self._version, "data_path": self._data_path, "voice": self._voice}
    ...
    def set_voice(self, voice_code):
        ...
        if "mb" in voice_code:
            ...
            available = {voice.identifier[3:]: voice.identifier for voice in self.available_voices("mbrola")}
        else:
    ...
    def synthetize(self, text: str):
        ...
        file_p = self._libc.fopen(self._tempfile.name.encode(), self._tempfile.mode.encode())

        self._espeak.set_phoneme_trace(0x01 << 4 | ord("_") << 8, file_p)
        status = self._espeak.synthetize(
            ctypes.c_char_p(text.encode("utf8")), ctypes.c_size_t(len(text) + 1), ctypes.c_uint(0x01)
        )
