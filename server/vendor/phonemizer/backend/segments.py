...
class SegmentsBackend(BaseBackend):
    ...
    def supported_languages():
        ...
        return {g2p.stem: g2p for g2p in directory.iterdir() if g2p.suffix == ".g2p"}
    ...
    def _load_g2p_profile(cls, language: str) -> segments.Profile:
        ...
        if not pathlib.Path(language).is_file():
            try:
                ...
                raise RuntimeError(f"grapheme to phoneme file not found: {language}") from None
        ...
        with open(language, "r", encoding="utf8") as flang:
            for num, line in enumerate(flang):
                ...
                if not len(elts) == 2:
                    raise RuntimeError(
                        "grapheme to phoneme file, line {} must have 2 rows but have {}: {}".format(
                            num + 1, len(elts), language
                        )
                    )
                g2p[elts[0]] = elts[1]

        # build the segments profile from the g2p mapping
        return segments.Profile(*[{"Grapheme": k, "mapping": v} for k, v in g2p.items()])
    ...
    def _phonemize_aux(self, text: List[str], offset: int, separator: Separator, strip: bool) -> List[str]:
        ...
        phonemized = (self._tokenizer(line, column="mapping", errors="strict") for line in text)
