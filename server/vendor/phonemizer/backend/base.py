...
class BaseBackend(abc.ABC):
    ...
    def __init__(
        ...
        self._logger = logger
        self._logger.info("initializing backend %s-%s", self.name(), ".".join(str(v) for v in self.version()))
    ...
    def _init_language(cls, language):
        ...
        if not cls.is_supported_language(language):
            raise RuntimeError(f'language "{language}" is not supported by the {cls.name()} backend')
        return language
    ...
    def phonemize(
        self, text: List[str], separator: Optional[Separator] = None, strip: bool = False, njobs: int = 1
    ) -> List[str]:
        ...
        if isinstance(text, str):
            ...
            raise RuntimeError("input text to phonemize() is str but it must be list of str")
    ...
    def _phonemize_postprocess(self, phonemized: List[str], punctuation_marks, separator: Separator, strip: bool):
