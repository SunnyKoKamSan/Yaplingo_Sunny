...
class FestivalBackend(BaseBackend):
    ...
    def __init__(
        ...
        super().__init__(
            language, punctuation_marks=punctuation_marks, preserve_punctuation=preserve_punctuation, logger=logger
        )
    ...
    def set_executable(cls, executable: str):
        ...
        if not (executable.is_file() and os.access(executable, os.X_OK)):
            raise RuntimeError(f"{executable} is not an executable file")
    ...
    def executable(cls) -> Path:
        ...
        if "PHONEMIZER_FESTIVAL_EXECUTABLE" in os.environ:
            executable = pathlib.Path(os.environ["PHONEMIZER_FESTIVAL_EXECUTABLE"])
            if not (executable.is_file() and os.access(executable, mode=os.X_OK)):
                raise RuntimeError(f"PHONEMIZER_FESTIVAL_EXECUTABLE={executable} is not an executable file")
            return executable.resolve()

        executable = shutil.which("festival")
        if not executable:  # pragma: nocover
            raise RuntimeError("failed to find festival executable")
    ...
    def version(cls):
        ...
        # we don't need
        long_version = subprocess.check_output([festival, "--version"]).decode("latin1").strip()
        ...
        try:
            ...
            raise RuntimeError(f"cannot extract festival version from {festival}") from None
    ...
    def _preprocess(cls, text: List[str]):
        ...
        """
        cleaned_text = (cls._cleaned(line) for line in text if line != "")

        return "\n".join(cls._double_quoted(line) for line in cleaned_text if line != "")
    ...
    def _run_festival(cmd: str, fstderr: IO) -> str:
        ...
        """
        try:
            output = subprocess.check_output(shlex.split(cmd, posix=False), stderr=fstderr)
            ...
            fstderr.seek(0)
            raise RuntimeError(
                f'Command "{cmd}" returned exit status {err.returncode}, output is:\n{fstderr.read()}'
            ) from None
    ...
    def _postprocess_word(cls, word: List[List[str]], separator: Separator, strip: bool) -> str:
        ...
        out = sep.join(cls._postprocess_syll(syll, separator, strip) for syll in word[1:])
        return out if strip else out + sep
    ...
    def _postprocess(cls, tree: str, separator: Separator, strip: bool) -> List[str]:
        ...
        return [
            cls._postprocess_line(line, separator, strip)
