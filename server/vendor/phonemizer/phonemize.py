...
def phonemize(  # pylint: disable=too-many-arguments
    ...
    if sys.version_info < (3, 6):  # pragma: nocover
        logger.error(
            "Your are using python-%s which is unsupported by the phonemizer, please update to python>=3.6",
            ".".join(sys.version_info),
        )

    # ensure the arguments are valid
    _check_arguments(backend, with_stress, tie, separator, language_switch, words_mismatch)
    ...
    if cache_key in _PHONEMIZER_CACHE:
        return _phonemize(
            _PHONEMIZER_CACHE[cache_key], text, separator, strip, njobs, prepend_text, preserve_empty_lines
        )
...
def _check_arguments(  # pylint: disable=too-many-arguments
    ...
    if with_stress and backend != "espeak":
        raise RuntimeError(
            'the "with_stress" option is available for espeak backend only, but you are using {} backend'.format(
                backend
            )
        )
    ...
    if tie and backend != "espeak":
        raise RuntimeError(
            'the "tie" option is available for espeak backend only, but you are using {} backend'.format(backend)
        )

    # tie option incompatible with phone separator
    if tie and separator.phone:
        raise RuntimeError(f'the "tie" option is incompatible with phone separator (which is "{separator.phone}")')

    # language_switch option only valid for espeak
    if language_switch != "keep-flags" and backend != "espeak":
        raise RuntimeError(
            'the "language_switch" option is available for espeak backend only, but you are using {} backend'.format(
                backend
            )
        )
    ...
    if words_mismatch != "ignore" and backend != "espeak":
        raise RuntimeError(
            'the "words_mismatch" option is available for espeak backend only, but you are using {} backend'.format(
                backend
            )
        )
