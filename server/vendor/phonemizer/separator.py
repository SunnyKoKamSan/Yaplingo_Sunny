...
class Separator:
    ...
    def __init__(self, word: str = " ", syllable: Optional[str] = None, phone: Optional[str] = None):
        ...
        if len(sep1) != len(sep2):
            raise ValueError(
                ...
                'phone="{}", must be all differents if not empty'.format(phone, syllable, word)
            )
    ...
    def __eq__(self, other: "Separator"):
        return self.phone == other.phone and self.syllable == other.syllable and self.word == other.word

    def __str__(self):
        return f'(phone: "{self.phone}", syllable: "{self.syllable}", word: "{self.word}")'
    ...
    def input_output_separator(self, field_separator: Union[str, bool]) -> Union[str, bool]:
        ...
        if isinstance(field_separator, str):
            if field_separator in self:
                raise RuntimeError(
                    f'cannot prepend input with "{field_separator}" because it is already a token separator: {self}'
                )
        ...
        raise RuntimeError(f"invalid input/output separator, must be bool or str but is{field_separator}")
