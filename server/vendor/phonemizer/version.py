...
from phonemizer.backend import EspeakBackend, EspeakMbrolaBackend, FestivalBackend, SegmentsBackend
...
def version():
    ...
    if EspeakBackend.is_available():
        available.append(
            "espeak-" + ("ng-" if EspeakBackend.is_espeak_ng() else "") + _version_as_str(EspeakBackend.version())
        )
    else:  # pragma: nocover
    ...
    if FestivalBackend.is_available():
        available.append("festival-" + _version_as_str(FestivalBackend.version()))
    else:  # pragma: nocover
        unavailable.append("festival")

    if SegmentsBackend.is_available():
        available.append("segments-" + _version_as_str(SegmentsBackend.version()))
    else:  # pragma: nocover
